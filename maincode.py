...
while flag:
    ...
    if cadastro.startswith('s'):
        ...
        nome = cadastrar_nome()
        data_de_nascimento = cadastrar_data()
    ...
    crud = input('Digite: (1)Para ver seus dados,'\
                '(2)Para editar seus dados,'\
                '(3)Para deletar seus dados,' \
                '(4)Para continuar ir para o questionário,'\
                '(5)Para ir para o tutorial'\
                '(6)Para ver seu feedback de desempenho.').lower().strip()
    
    if crud.startswith('1'):
        print(nome)
